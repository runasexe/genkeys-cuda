...
#ifndef ED25519_CUDA_FE_CU
#define ED25519_CUDA_FE_CU

static __device__ __host__ inline uint64_t load_3(const unsigned char *in) {
    uint64_t result;
...
static __device__ __host__ inline uint64_t load_4(const unsigned char *in) {
    uint64_t result;
    result = (uint64_t) in[0];
    result |= ((uint64_t) in[1]) << 8;
    result |= ((uint64_t) in[2]) << 16;
